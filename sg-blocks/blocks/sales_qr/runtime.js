...
async function drawQr(canvas, fallbackEl, text){
  ...
  // IMPORTANT: encode the FULL deep_link (https://t.me/...) so camera detects it as URL
  const imgUrl =
    `https://quickchart.io/qr?size=${encodeURIComponent(String(cssSize))}` +
    `&margin=1&ecLevel=H&text=${encodeURIComponent(text)}`;
...
export function mount(rootEl, props={}, ctx={}){
  ...
  const debugEl  = $(root, '[data-sq-debug]');
  ...
  function setDebug(s){ if (debugEl) debugEl.textContent = String(s||''); }
  ...
  async function refresh(){
    ...
    try{
      ...
      // Accept multiple key spellings (in case of changes)
      const deep = r && (r.deep_link || r.deepLink || r.link) ? String(r.deep_link || r.deepLink || r.link) : '';
      const token = r && r.token ? String(r.token) : '';
      const bot = r && r.bot_username ? String(r.bot_username).replace(/^@/,'').trim() : '';

      // If deep_link is missing but bot+token exist — build URL client-side
      lastLink = deep || '';
      if (!lastLink && bot && token){
        lastLink = `https://t.me/${bot}?start=sale_${token}`;
      }
      if (!lastLink){
        // fallback: token only (will show as search in camera)
        lastLink = token || '';
      }
