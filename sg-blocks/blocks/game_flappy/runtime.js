...
//
// Чистый модуль блока. Ничего не делает при импорте.
// Лоадер Студии вызывает mount(root, props, ctx) и получает cleanup().
...
export async function mount(root, props = {}, ctx = {}) {
  ...
  const BASE = String(ctx.base_url || '').replace(/\/?$/, '/'); // .../blocks/game_flappy/

  const hapticsOn = props.haptics !== false;
  ...
  // HUD (не обязателен)
  const coinCountEl = host.querySelector('#fl-coin-count');
  ...
  const GAP_MIN = num(props.gap_min, 140);
  const GAP_MAX = num(props.gap_max, 220);
  ...
  const GAME_ID = String(props.game_id || 'flappy');
  const SUBMIT_MODE = String(props.submit_mode || 'daily');

  // -------- assets (через base_url) ----------
  const ASSETS = {
    bird: {
      img:
        props.bird_mode === 'custom' && props.bird_img
          ? props.bird_img
          : BASE + 'assets/bird_default.png',
      w: 56,
      h: 42
    },
    pipes: {
      top: BASE + 'assets/pipe_top.png',
      bottom: BASE + 'assets/pipe_bottom.png',
      width: 54
    },
    coin: { img: BASE + 'assets/coin.png', w: 32, h: 32, value: COIN_VALUE },
    shield: { img: BASE + 'assets/shield.png', w: 34, h: 34, dur_ms: SHIELD_DURATION }
  };
  ...
  let pipes = []; // {x, gapY, gapH, passed, elTop, elBot}
  let coins = []; // {x,y,w,h,el,value}
  let powers = []; // shield power-ups {x,y,w,h,el}
  ...
  setText(bestEl, best);
  setText(worldEl, WORLD_RECORD);
  if (coinCountEl) setText(coinCountEl, 0);
  setTimeBar(1);
  hide(resBox);
  show(hintEl);
  hide(cta);
  ...
  function onCta(e) {
    ...
    if (!btn) return; // рестарт только по кнопке
    e.preventDefault();
    e.stopPropagation();
    resetScene();
    running = true;
  ...
  if (AUTOSTART) {
    // лёгкий автостарт для промо, но без сабмита если нет TG
    start();
  }
  ...
  function resetActors() {
    // clear pipes/coins/powers DOM
    pipes.forEach(p => {
      p.elTop && p.elTop.remove();
      p.elBot && p.elBot.remove();
    });
    coins.forEach(c => c.el && c.el.remove());
    powers.forEach(p => p.el && p.el.remove());
    pipes.length = 0;
    coins.length = 0;
    powers.length = 0;

    bird = {
      x: Math.max(60, world.w * 0.25),
      y: world.h * 0.5,
      w: ASSETS.bird.w,
      h: ASSETS.bird.h,
      vy: 0,
  ...
  function tick(t) {
    const prev = tick._prev || t;
    tick._prev = t;
    let dt = (t - prev) / 1000;
    if (dt > 0.05) dt = 0.05;

    // ускорение каждые ACCEL_EACH_MS
    if (t - lastAccelAt >= ACCEL_EACH_MS) {
      SPEED_X += SPEED_STEP;
      lastAccelAt = t;
    }
    ...
    timeLeft = Math.max(0, SESSION_MS - elapsed);
    setTimeBar(timeLeft / SESSION_MS);
    ...
    // земля/потолок
    if (bird.y + bird.h * 0.5 > world.floorY) {
      bird.y = world.floorY - bird.h * 0.5;
      collide('ground');
    }
    if (bird.y - bird.h * 0.5 < 0) {
      bird.y = bird.h * 0.5;
      bird.vy = 0;
    }
    placeEl(birdEl, bird.x, bird.y, bird.w, bird.h);
  ...
  function spawnPipes() {
    ...
    if (pipes.length === 0 || lastX < world.w) {
      // создать новую пару, если следующая точка спавна видна
      if (spawn.nextPipeX <= world.w) {
        const gapH = randInt(GAP_MIN, GAP_MAX);
        const gapY = randInt(gapH * 0.6, world.h - SAFE_FLOOR_PAD - gapH * 0.6);
        const x = world.w + randInt(160, 240);

        const elTop = make('div', 'fl-pipe-part'); // верхняя
        const elBot = make('div', 'fl-pipe-part'); // нижняя
        elTop.style.backgroundImage = `url(${ASSETS.pipes.top})`;
        elBot.style.backgroundImage = `url(${ASSETS.pipes.bottom})`;

        stage.appendChild(elTop);
        stage.appendChild(elBot);

        const p = { x, gapY, gapH, passed: false, elTop, elBot };
        pipes.push(p);
        positionPipe(p);

        // шанс монеты в зазоре
        if (Math.random() < COIN_IN_GAP_PROB) {
          const cy = gapY; // центр зазора
          const cx = x + ASSETS.pipes.width * 0.5;
          const el = make('div', 'fl-coin');
          el.style.backgroundImage = `url(${ASSETS.coin.img})`;
          stage.appendChild(el);
          coins.push({ x: cx, y: cy, w: ASSETS.coin.w, h: ASSETS.coin.h, el, value: COIN_VALUE });
          placeEl(el, cx, cy, ASSETS.coin.w, ASSETS.coin.h);
        }

        // шанс щита, если КД прошёл
        if (performance.now() > bird.shieldCdTill && Math.random() < SHIELD_PROB) {
          const py = clamp(gapY + randInt(-gapH * 0.35, gapH * 0.35), 30, world.floorY - 30);
          const px = x + randInt(40, 120);
          const el = make('div', 'fl-power');
          el.style.backgroundImage = `url(${ASSETS.shield.img})`;
          stage.appendChild(el);
  ...
  function positionPipe(p) {
    ...
    // top: от верха до gapY - gapH/2
    const topH = Math.max(0, p.gapY - p.gapH * 0.5);
    // bottom: от низа наверх
  ...
  function stepPipesCoinsPowers(dt) {
    ...
    for (let i = pipes.length - 1; i >= 0; i--) {
      ...
      // подсчёт очков, когда пролетели центр трубы
      if (!p.passed && p.x + ASSETS.pipes.width * 0.5 < bird.x) {
        p.passed = true;
        score += 1;
        setText(scoreEl, score);
        haptic('success');
      ...
      // коллизии (если нет щита)
      if (!hasShield()) {
        if (intersectsPipe(bird, p)) {
          collide('pipe');
          break;
        }
      }
    ...
    for (let i = coins.length - 1; i >= 0; i--) {
      const c = coins[i];
      c.x += vx;
      placeEl(c.el, c.x, c.y, c.w, c.h);

      if (intersects(bird, c)) {
        coinsTaken += c.value || COIN_VALUE;
        if (coinCountEl) setText(coinCountEl, coinsTaken);
        c.el.remove();
        coins.splice(i, 1);
        haptic('light');
        continue;
    ...
    for (let i = powers.length - 1; i >= 0; i--) {
      const pw = powers[i];
      pw.x += vx;
      placeEl(pw.el, pw.x, pw.y, pw.w, pw.h);
    ...
    // щит таймаут
    if (bird.shieldTill && performance.now() > bird.shieldTill) {
      bird.shieldTill = 0;
  ...
  function hasShield() {
    return bird.shieldTill && performance.now() < bird.shieldTill;
  }
  ...
  function updateShieldBar() {
    ...
    let v = 0;
    if (hasShield()) {
      const left = bird.shieldTill - performance.now();
      v = clamp(left / SHIELD_DURATION, 0, 1);
    }
    shieldBar.style.transform = `scaleX(${v.toFixed(3)})`;
    shieldBar.style.transformOrigin = 'left center';
  }

  function collide(kind) {
    if (!running) return;
    if (hasShield()) {
      // поглощает один удар, снимаем щит досрочно
      bird.shieldTill = 0;
      birdEl.classList.remove('fl-bird--shield');
      updateShieldBar();
      haptic('error');
  ...
  function finish() {
    ...
    // итоговый счёт = трубы + монеты
    const finalScore = score + coinsTaken;

    // обновить локальный best
    if (finalScore > best) {
      best = finalScore;
      saveBest(best);
    }
    ...
    // попытка сабмита (в превью просто молча скипнется)
    submitScore(finalScore).catch(() => {});
  }

  // -------- network (safe no-op в превью) --------
  async function submitScore(finalScore) {
    if (!TG || !publicId) return; // превью: тут выходим
    const init_data = TG.initData || '';
    const u = (TG.initDataUnsafe && TG.initDataUnsafe.user) || null;
    if (!(init_data && u && u.id)) return;

    const url = `/api/public/app/${encodeURIComponent(publicId)}/event`;
    const payload = {
      type: 'game.submit',
      init_data,
      tg_user: {
      ...
      payload: { game_id: GAME_ID, mode: SUBMIT_MODE, score: Number(finalScore || 0) }
    };
  ...
  function layout() {
    ...
    world.w = Math.max(300, Math.floor(r.width));
    world.h = Math.max(360, Math.floor(r.height));
    world.floorY = world.h - SAFE_FLOOR_PAD;
    // применим текущие позиции
    placeEl(birdEl, bird.x, bird.y, bird.w, bird.h);
  ...
  function setTimeBar(frac) {
    ...
    timeBar.style.transform = `scaleX(${v.toFixed(3)})`;
    timeBar.style.transformOrigin = 'left center';
  }
  ...
  function placeEl(el, cx, cy, w, h) {
    // позиционируем по центру (cx, cy)
    el.style.width = w + 'px';
    el.style.height = h + 'px';
    el.style.transform = `translate(${Math.round(cx - w * 0.5)}px, ${Math.round(
      cy - h * 0.5
    )}px)`;
  }
  function placeRect(el, cx, h, w, height) {
    // для труб, где удобнее давать высоту целиком
    el.style.width = w + 'px';
  ...
  function intersectsPipe(a, p) {
    // верхний прямоугольник
    const w = ASSETS.pipes.width;
    const topRect = { x: p.x, y: p.gapY * 0.5, w, h: p.gapY };
    // нижний прямоугольник
    const botH = Math.max(0, world.floorY - (p.gapY + p.gapH * 0.5));
    const botRect = {
      x: p.x,
      y: p.gapY + p.gapH * 0.5 + botH * 0.5,
      w,
      h: botH
    };
    return intersects(a, topRect) || intersects(a, botRect);
  }
  ...
  function applyAssets() {
    // размеры через CSS custom props корня документа
    ...
    doc.documentElement.style.setProperty('--pipe-w', (ASSETS.pipes.width || 54) + 'px');
  ...
  function saveBest(v) {
    try {
      localStorage.setItem('flappy_best', String(v));
    } catch (_) {}
  }
  ...
  function cleanup() {
    ...
    try {
      cta.removeEventListener('click', onCta, true);
    } catch (_) {}
    try {
      ro.disconnect();
    } catch (_) {}

    // почистим динамические элементы
    try {
      pipes.forEach(p => {
        p.elTop && p.elTop.remove();
        p.elBot && p.elBot.remove();
