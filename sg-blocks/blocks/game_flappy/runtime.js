...
// Строгий порт превью-игры под формат блока. export mount(...) -> cleanup()
...
export async function mount(root, props = {}, ctx = {}) {
  ...
  // ==== применяем ассеты (в scope host, чтобы не трогать другие блоки)
  function applyAssets() {
    ...
    if (coinIco && ASSETS.coin.img)
      coinIco.style.backgroundImage = `url(${ASSETS.coin.img})`;
    if (shIco && ASSETS.shield.img)
      shIco.style.backgroundImage = `url(${ASSETS.shield.img})`;
  }

  // ==== настройки (как в превью)
  ...
  const haptic = (lvl) => {
    try {
      TG?.HapticFeedback?.impactOccurred(lvl || 'light');
    } catch (_) {}
  };
  ...
  function spawnPipe() {
    ...
    // shield (с учётом КД)
    if (
      Date.now() - lastShieldSpawn > SHIELD_COOLDOWN &&
      Math.random() < SHIELD_PROB
    ) {
      const s = doc.createElement('div');
      s.className = 'fl-power';
      if (ASSETS.shield.img)
        s.style.backgroundImage = `url(${ASSETS.shield.img})`;
      stage.appendChild(s);
      const it = {
        type: 'shield',
        x: p.x + 300,
  ...
  function collidePipe() {
    ...
    for (const p of pipes) {
      if (
        rectsOverlap(br, p.topEl.getBoundingClientRect()) ||
        rectsOverlap(br, p.botEl.getBoundingClientRect())
      )
        return true;
    }
    return false;
  ...
  async function finish() {
    ...
    // мягкий сабмит (в превью просто не выполнится — ок)
    try {
  ...
  function resetScene() {
    ...
    if (hintEl) hintEl.style.display = ''; // показать «Тапни…»
  ...
  function tick() {
    ...
    const dt = Math.min(32, now - (tick._prev || now));
    tick._prev = now;
    ...
    // GC
    while (pipes.length && pipes[0].x < -(ASSETS.pipes.width || 54) - 2) {
      removePipe(pipes[0]);
      pipes.shift();
    }
  ...
  // дублируем для гарантии первого старта
  hintEl && hintEl.addEventListener('pointerdown', onPointer, { passive: false });
  const hostFirstStart = (e) => {
    if (!started) onPointer(e);
  };
  host.addEventListener('pointerdown', hostFirstStart, {
    passive: false,
    capture: true
  });
  ...
  const onKey = (e) => {
    ...
    if (e.key === 'Escape') {
      // мягко игнорируем в превью
    }
  };
  doc.addEventListener('keydown', onKey);

  // CTA — возвращаемся в IDLE (без автозапуска), показываем хинт
  const onCta = (e) => {
    ...
    resBox && resBox.classList.remove('show');
    cta && cta.classList.remove('show');

    resetScene();        // started=false, hint виден
    running = true;      // включим цикл, чтобы шла «левитация»
    try {
      win.cancelAnimationFrame(raf);
    } catch (_) {}
    tick._prev = performance.now();
  ...
  // старт (idle): показываем подсказку и запускаем цикл для левитации
  applyAssets();
  layout();
  resetScene();     // started=false, hint виден, прогрессбар = 1
  running = true;   // нужен цикл для idle-анимации
  tick._prev = performance.now();
  raf = win.requestAnimationFrame(tick);
  ...
  function cleanup() {
    ...
    try {
      host.removeEventListener('pointerdown', hostFirstStart, true);
    } catch (_) {}
    try {
